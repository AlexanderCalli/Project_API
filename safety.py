...
def get_images(ws, workflow, update_queue):
    prompt_id = queue_prompt(workflow)['prompt_id']
    output_images = {}
    start_time = time.time()
    last_update_time = start_time
    update_queue.put({"status": "processing", "message": "Image generation started"})
    ...
    while True:
        ...
        if current_time - last_update_time >= 1:
            update_queue.put({"status": "processing", "message": "Image generation in progress"})
            last_update_time = current_time

    update_queue.put({"status": "processing", "message": "Image generation completed"})
    ...
    def generate(data):
        ...
        def process_image():
            try:
                ...
                images = get_images(ws, workflow, update_queue)
                ...
                for node_id in images:
                    for i, image_data in enumerate(images[node_id]):
                        ...
                        update_queue.put({"status": "processing", "message": f"Processed image {i+1}"})

                # Send the final update with results
                update_queue.put({
                    "status": "completed",
                ...
                update_queue.put({"status": "error", "message": str(e)})

            finally:
                update_queue.put(None)  # Signal the end of updates
        ...
        while True:
            ...
            if update is None:
                break
            yield send_sse_message(update)

    return Response(generate(request_data), mimetype='text/event-stream')
